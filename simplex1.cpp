...
const std::size_t N = 5, M = 5;
typedef std::array<std::array<double, M>, N> Arr;
 
void print(const Arr& table);
bool isContainNeg(const std::array<double, M>& table);
...
void print(const Arr& table)
{
    ...
    for (std::size_t i = 1; i < N; i++)
    {
        ...
        std::cout << "Значение целевой функции " << table[N - 1][1] << '\n';
}
